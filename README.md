...
**核心功能 MIT 开源，商用也免费；企业级功能闭源，单独授权。**

你在这里能拿到全部让代理跑起来的能力——三种客户端协议、上游供应商抽象、
流式转发、工具调用模拟、AOT 单文件发布。个人和小团队用这些就够了，不问自取。

企业版（另仓、闭源）解决的是"跑起来之后管不住"的问题：多租户与 API Key 管理、
配额与成本核算、管理后台、审计日志与合规留存、SSO 集成、可观测导出、多实例协同。

详见 [docs/licensing.md](docs/licensing.md)。

**需要商业授权请联系：yangfan.jr@outlook.com**
